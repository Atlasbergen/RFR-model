...
class Molecule:
    ...
    def D_eff(self, T, P, C_T, C_main, Conc, Molecules):
        ...
        for i in comb:
            denom += (i[0]/C_T)/D_AB(T, P, self.T_boil, i[1].T_boil, self.Vb, i[1].Vb, self.M_w, i[1].M_w)
    ...
    def r(self, T, C_A, C_B, C_C, C_D, C_E, C_F, C_G):
        ...
        if reaction_name == "reaction_1":
            return (
                ...
                * (
                    (Reaction.k(Reaction.A_O2, T, Reaction.Ea_O2) * ((C_B*R*T/101325)) ** 0.5)
                    / (1 + (Reaction.k(Reaction.A_O2, T, Reaction.Ea_O2) * ((C_B*R*T/101325)) ** 0.5))
                )
            )
            ...
            return Reaction.k(Reaction.A_DMEHCHO, T, Reaction.Ea_DMEHCHO)*(Reaction.k(Reaction.A_DME, T, Reaction.Ea_DME)*(C_F*R*T/101325)/(1 + Reaction.k(Reaction.A_DME, T, Reaction.Ea_DME)*(C_F*R*T/101325)))*((Reaction.k(Reaction.A_O2, T, Reaction.Ea_O2) * (abs(C_B*R*T/101325)) ** 0.5)/(1 + (Reaction.k(Reaction.A_O2, T, Reaction.Ea_O2) * (C_B*R*T/101325) ** 0.5)))
        else:
