...
for i in range(1, 401):
    ...
    my_path = "data07/output_rev_data_" + str(i) + ".txt"
    np.savetxt(my_path, sol.y)
    ...
    uinit[6*m:7*m] = C_G[::-1]
    uinit[7*m:8*m] = T_b[::-1]
    uinit[8*m:9*m] = C_As[::-1]
    uinit[9*m:10*m] = C_Bs[::-1]
    uinit[10*m:11*m] = C_Cs[::-1]
    uinit[11*m:12*m] = C_Ds[::-1]
    uinit[12*m:13*m] = C_Es[::-1]
    uinit[13*m:14*m] = C_Fs[::-1]
    uinit[14*m:15*m] = C_Gs[::-1]
    uinit[15*m:16*m] = T_s[::-1]
